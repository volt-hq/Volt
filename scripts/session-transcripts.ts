 ...
 *   --analyze      Spawn volt subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("volt", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn volt: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".volt/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn volt subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".volt/agent/AGENTS.md");
