...
5. Check the changeset fragment, not the changelog. PRs that change product source (`packages/*/src`) must include a `.changeset/*.md` fragment (CI enforces this); `CHANGELOG.md` itself is generated at release time and must not be edited. Review the fragment against `.changeset/README.md`: the kind and area fit the change, the sentence describes user-visible behavior rather than implementation, `breaking` fragments use a minor bump with migration guidance, pure refactors use `internal`, and external contributions carry `([#N](url) by [@user](url))` attribution.
