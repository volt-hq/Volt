...
<group n="3" title="iOS discovery, palette, and invocation">
  ...
  <item ref="C.3" status="resolved" prereq="C.2,B.4">
    ...
    <evidence>Resolved 2026-06-23: added the iOS chat command palette sheet for discovered descriptors with search, category/source grouping, loading/empty/error states, host-disabled reason display, unsupported-argument disabling, source-specific visual treatment, direct no-arg invocation, single-string argument prompting, and connected/enabled guards around `invoke_ui_action`; covered by `VoltSessionLifecycleTests` invocation/disabled-action tests and `XcodeProjectConfigurationTests` source-level UI affordance checks; verified with `swift test --filter VoltSessionLifecycleTests`, `swift test --filter XcodeProjectConfigurationTests`, `swift test` from `../volt-app/Packages/VoltClient`, `xcodebuild -scheme Volt -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=27.0' test` from `../volt-app`, and `git diff --check -- Volt/ChatView.swift Packages/VoltClient/Sources/VoltCore/VoltSession.swift Packages/VoltClient/Tests/VoltCoreTests/VoltSessionLifecycleTests.swift Packages/VoltClient/Tests/VoltCoreTests/XcodeProjectConfigurationTests.swift`; app commit 4657cdabc88d6992920b24d69f59b51dc833b250.</evidence>
  </item>

  <item ref="C.4" status="resolved" prereq="C.3,B.5">
    ...
    <evidence>Resolved 2026-06-23: added `VoltSession` native action pending state, guarded overlapping commands while an action disposition is pending, selected queued `streamingBehavior` from descriptor support during active streams, handled `accepted`, `queued`, `completed`, `handled`, `cancelled`, malformed, and failed `invoke_ui_action` responses without deadlocking on terminal statuses or falsely ending unrelated streams, honored action/state refresh hints, and updated the demo mock action to return `handled`; covered by `VoltSessionLifecycleTests` for direct invocation encoding, handled extension-command completion without `agent_end`, terminal completed/cancelled responses, accepted prompt-like streaming through `agent_end`, queued follow-up behavior while streaming, and failure response cleanup; verified with `swift test --filter VoltSessionLifecycleTests`, `swift test` from `../volt-app/Packages/VoltClient`, `xcodebuild -scheme Volt -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=27.0' test` from `../volt-app`, and `git diff --check -- Packages/VoltClient/Sources/VoltCore/VoltSession.swift Packages/VoltClient/Sources/VoltClient/Transport/MockVoltTransport.swift Packages/VoltClient/Tests/VoltCoreTests/VoltSessionLifecycleTests.swift`; app commit be4f3e849a89e82a00a3d4f3e7ffdafc1b9f0566.</evidence>
  </item>
</group>
...
<group n="5" title="Richer action UX, arguments, and extensions">
  ...
  <item ref="E.2" status="resolved" prereq="C.4,D.3,D.4">
