...
`npm-shrinkwrap.json`; npm-installed dependencies retain the license files
shipped in their own packages.
...
distribution under `MIT OR Apache-2.0`. It is intentionally **not** included
in the standalone Bun binary, which consequently cannot host `volt daemon`
or provide remote/iOS access.
