...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@mariozechner", "volt-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@mariozechner+volt-coding-agent@0.0.0",
			"node_modules",
			"@mariozechner",
			"volt-coding-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "volt-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env[pathEnvKey] = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.VOLT_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "volt-coding-agent", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@earendil-works+volt-coding-agent@0.67.68\\node_modules\\@earendil-works\\volt-coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@earendil-works/volt-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @earendil-works/volt-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@earendil-works/volt-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@earendil-works/volt-coding-agent")).toBe(
			"Update @earendil-works/volt-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@earendil-works/volt-coding-agent");
		...
		expect(command).toEqual({
			...
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @earendil-works/volt-coding-agent`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@earendil-works/volt-coding-agent", undefined, "@new-scope/volt");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/volt"],
			display: `npm --prefix ${prefix} uninstall -g @earendil-works/volt-coding-agent && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/volt`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@earendil-works/volt-coding-agent"],
					display: `npm --prefix ${prefix} uninstall -g @earendil-works/volt-coding-agent`,
				},
				{
					command: "npm",
					args: [
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@earendil-works/volt-coding-agent", ["npm", "--prefix", configuredPrefix]);
		...
		expect(command).toEqual({
			...
			display: `npm --prefix ${configuredPrefix} install -g --ignore-scripts --min-release-age=0 @earendil-works/volt-coding-agent`,
		});
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @earendil-works/volt-coding-agent`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@earendil-works\\volt-coding-agent";
		process.env.VOLT_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@earendil-works/volt-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @earendil-works/volt-coding-agent",
		);
	});
	...
	test.runIf(process.platform !== "win32")("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@earendil-works/volt-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @earendil-works/volt-coding-agent",
		});
	});
	...
	test.runIf(process.platform !== "win32")(
		...
		() => {
			...
			expect(command).toEqual({
				...
				display:
					"pnpm remove -g @earendil-works/volt-coding-agent && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/volt",
				steps: [
					{
						command: "pnpm",
						args: ["remove", "-g", "@earendil-works/volt-coding-agent"],
						display: "pnpm remove -g @earendil-works/volt-coding-agent",
					},
					{
						command: "pnpm",
						args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/volt"],
	...
	test.runIf(process.platform !== "win32")("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@earendil-works/volt-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@earendil-works", "volt-coding-agent");
	...
	test.runIf(process.platform !== "win32")(
		...
		() => {
			...
			expect(command).toEqual({
				...
				display:
					"yarn global remove @earendil-works/volt-coding-agent && yarn global add --ignore-scripts @new-scope/volt",
				steps: [
					{
						command: "yarn",
						args: ["global", "remove", "@earendil-works/volt-coding-agent"],
						display: "yarn global remove @earendil-works/volt-coding-agent",
					},
					{
						command: "yarn",
						args: ["global", "add", "--ignore-scripts", "@new-scope/volt"],
	...
	test.runIf(process.platform !== "win32")(
		...
		() => {
			...
			expect(command).toEqual({
				...
				display:
					"bun uninstall -g @earendil-works/volt-coding-agent && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/volt",
				steps: [
					{
						command: "bun",
						args: ["uninstall", "-g", "@earendil-works/volt-coding-agent"],
						display: "bun uninstall -g @earendil-works/volt-coding-agent",
					},
					{
						command: "bun",
						args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/volt"],
	...
	test.runIf(process.platform !== "win32")("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@earendil-works/volt-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@earendil-works/volt-coding-agent")).toContain(
