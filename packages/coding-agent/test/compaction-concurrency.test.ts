...
function isTurnPrefixRequest(context: Context): boolean {
	...
	return (
		Array.isArray(content) && content.some((part) => part.type === "text" && part.text.includes("PREFIX of a turn"))
	);
}
...
describe("split-turn compaction requests", () => {
	it("serializes history and turn-prefix summaries", async () => {
		...
		faux.setSimpleResponses([
			async (context) => {
				expect(isTurnPrefixRequest(context)).toBe(false);
				activeRequests++;
			...
			(context) => {
				expect(isTurnPrefixRequest(context)).toBe(true);
				activeRequests++;
				maxActiveRequests = Math.max(maxActiveRequests, activeRequests);
	...
	it("retries the history summary before starting the turn-prefix summary", async () => {
		...
		expect(result.summary).toContain("history summary");
		expect(result.summary).toContain("prefix summary");
		expect(requests).toEqual(["history", "history", "prefix"]);
