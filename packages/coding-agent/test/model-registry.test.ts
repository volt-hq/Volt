...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@earendil-works/volt-ai";
import { getApiProvider } from "@earendil-works/volt-ai";
import { getOAuthProvider } from "@earendil-works/volt-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `volt-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
