 ...
 * 1. Volt starts with settings.json containing packages: ["npm:some-pkg"]
...
describe("SettingsManager - External Edit Preservation", () => {
	...
	it("should preserve file changes to packages array when changing unrelated setting", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				theme: "dark",
				packages: ["npm:volt-mcp-adapter"],
			}),
		);

		// Volt starts up, loads settings into memory
		const manager = SettingsManager.create(projectDir, agentDir);

		// At this point, globalSettings.packages = ["npm:volt-mcp-adapter"]
		expect(manager.getPackages()).toEqual(["npm:volt-mcp-adapter"]);
