...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `volt-sdk-attribution-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		cwd = join(tempDir, "project");
		agentDir = join(tempDir, "agent");
		mkdirSync(cwd, { recursive: true });
		mkdirSync(agentDir, { recursive: true });
		originalTelemetryEnv = process.env.VOLT_TELEMETRY;
		delete process.env.VOLT_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.VOLT_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["X-OpenRouter-Title"]).toBe("volt");
	...
	it("adds default attribution headers for Vercel AI Gateway models", async () => {
		...
		expect(headers?.["http-referer"]).toBe("https://github.com/volt-hq/Volt");
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe("Volt");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("volt");
	});
