...
function reviewRerunArgs(record: ReviewRunRecord, scopeMode: "incremental" | "full"): Record<string, unknown> {
	const identity = record.target.identity;
	return {
		...(identity.kind === "branch" && identity.baseCommit ? { base: identity.baseCommit } : {}),
		...(identity.kind === "pr" && identity.pullRequest ? { number: String(identity.pullRequest.number) } : {}),
		...(identity.kind === "commit" && identity.headCommit ? { ref: identity.headCommit } : {}),
		...(record.options.focus ? { focus: record.options.focus } : {}),
		...(record.options.scope.length > 0 ? { scope: record.options.scope.join(",") } : {}),
		effort: record.options.effort,
		includeOptional: record.options.includeOptional,
		scopeMode,
	};
}
	...
	switch (command.type) {
		...
		case "rerun_review": {
			...
			const response = await BUILTIN_HOST_ACTION_REGISTRY.invoke(
				...
				context.createHostActionContext(),
				reviewRerunArgs(record, command.mode ?? "incremental"),
				{ requireRemoteSafe: options.requireRemoteSafeUiActions },
			);
