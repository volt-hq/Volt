...
- Delegate only when the user or applicable project instructions explicitly request it, or when a concrete, self-contained task can be completed independently in an isolated context and the benefit from specialization or isolation materially outweighs the startup, latency, and coordination cost.
- Keep immediate blockers, tightly coupled steps, simple exploration, and work already underway with the root agent.
- Before proactively spawning, identify the specific required output and why the subagent's specialization or isolated context is worth that cost. If that benefit is not clear, do the task yourself.
