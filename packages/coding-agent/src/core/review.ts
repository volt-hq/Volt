...
export async function prepareReviewWorkflow(options: PrepareReviewWorkflowOptions): Promise<PreparedReviewWorkflow> {
	...
	return {
		...
		startedAt: Date.now(),
		incrementalPlan: planIncrementalReview(options.sessionManager, resolution, controls),
...
export async function runReview(options: RunReviewOptions): Promise<ReviewRunResult> {
	...
	try {
		...
		const snapshotTools = createReviewSnapshotTools(snapshot, tracker);
		const sharedTools = [...snapshotTools, ...inherited];
		...
		const candidateReport = await runReviewPass({
			...
			customTools: [...sharedTools, candidateCollector.tool],
		...
		const verificationReport = await runReviewPass({
			...
			customTools: [...sharedTools, verificationCollector.tool],
		...
		const parsed = buildParsedReview({
			...
			validatedCandidates,
			verificationReport,
			observedCoverage: tracker.snapshot(),
...
export async function runReviewWorkflow(options: ReviewWorkflowOptions): Promise<ReviewWorkflowResult> {
	...
	if (options.requireConfirmation) {
		...
		if (!confirmed) {
			...
			if (options.session.sessionManager) appendReviewRun(options.session.sessionManager, record);
			await resolution.dispose();
