...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@earendil-works/volt-coding-agent";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@earendil-works/volt-tui";
...
export default function (volt: ExtensionAPI) {
	...
	// -----------------------------------------------------------------------
	volt.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	volt.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	volt.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			volt.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						volt.appendEntry(SAVE_TYPE, getBoardDetails());
