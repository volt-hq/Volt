...
Search terms come from bounded request, recent conversation, and tool text, prioritizing observed search/symbol arguments and file names. Explicit paths, cited lines, foreground tool paths and changed-file lists take precedence over generic discovery. Git output is observed from foreground tools; this extension does not execute Git. Known paths avoid broad scans; otherwise a selected term narrows path discovery. Secondary search terms require at least 0.15 probability. Navigation choices come from observed native symbols. Jev selects finite candidates; it cannot generate commands, paths, arguments, or code. The ordinary main agent retains implementation and verification work.

Path observations retain their origin. Relative imports and Markdown links resolve against their containing source file; arbitrary source strings and fenced documentation examples do not become paths. Foreground find/grep output resolves against its search directory; a grep hit retains its line instead of being replaced by a generic file-start location. Before scoring mentions, native file discovery checks up to three containing directories and retains exact matches. Broad discovery also probes one observed extensionless path as a directory, so an implementation named `index.ts` can accompany matching test files even though native path normalization removes trailing directory separators. An extensionless file can consume that probe without yielding children; discovery remains partial. Unchecked or missing mentions are omitted; when path discovery is not available as a service, only explicit user paths and observed foreground paths are eligible.

Diff hunk locations guide reads past the file prefix. Single-line symbol locations expand into windows of up to 120 lines. Region choices favor named declarations and methods, excluding variable aliases and anonymous callbacks. Symbols are ranked for matching search terms before limiting the region menu. Up to two file-scoped native searches use up to eight identifiers extracted from observed foreground search/symbol arguments, reaching deeper methods without first collecting unrelated declarations. With no observed query, declaration, method, and test matches supplement missing or partial JavaScript/TypeScript symbols. These are bounded text matches, not complete syntax analysis. When declarations are available, a generic line-one choice is omitted; observed locations past line one remain eligible. Windows overlapping a foreground read start at their first unread line. A single remaining region requires no Jev focus call. A semantic lookup that adds no evidence does not trigger another assessment.
...
Previously offered regions are remembered separately from foreground reads, preventing repeated preparation of the same source location or skill. A complete native excerpt reaching EOF also suppresses later discovery of its covered range. These records do not establish delivery or model use. Mutating or unknown tools and changed service/skill catalogs clear the offer history, allowing refreshed preparation. An expired excerpt is absent from later request payloads; the main agent can still read its source when needed. Reports distinguish retirement after a foreground read from retirement at the offer limit.
...
The request snapshot is capped at 8,192 bytes, recent conversation at eight text messages / 8,192 bytes from the latest sixteen branch entries, and tool observations at four / 2,048 bytes each, with search/symbol arguments capped at 256 bytes per observation. Duplicate tool text is omitted from the recent-conversation window. Up to 64 relative paths can also be extracted from the first 8,192 bytes of each foreground result and observed tool arguments. Up to 64 read ranges describe evidence already available to the foreground; up to 72 separately recorded offers describe prior preparation. Context before a compaction boundary is skipped. Thinking blocks, system messages, custom session entries, image bytes, host resource IDs, and the absolute cwd are not automatically added. Those values could still appear in user-authored text or source content. Missing or truncated context can cause incorrect decisions.
...
Audits are appended at `agent_settled`, after automatic retries, compaction recovery, and queued continuations finish, or during safe shutdown/idle disable. An intermediate `agent_end` records observations without sealing the current audit. When delivered steering or follow-up input replaces a request scope, its audit is frozen with reason `superseded` and buffered alongside subsequent scopes until settlement. This preserves every scope without moving the canonical cursor during request collection and invalidating prepared context. The final scope uses reason `agent_settled`.

An abrupt process crash before the buffered audits are appended or SQLite persistence finishes can lose those audits. Work still running at sealing is marked `interrupted`, missing results remain unobserved, and late callbacks cannot mutate saved records or write into another branch. Audit append failures warn without blocking the main response. Normal session close drains SQLite persistence.
...
node node_modules/vitest/dist/cli.js --run test/jev-ahead-of-model.test.ts test/suite/jev-ahead-of-model.test.ts test/suite/agent-settled.test.ts
```
...
Settlement validation on 2026-09-22: 152 focused tests and the full repository check passed, including the command above and the existing retry-event, managed-work, and tool-response retry suites. Regressions cover recovery followed by more preparation, exhausted retries, abort during backoff, separately preserved steering/follow-up scopes, compaction recovery, awaited settlement handlers, and error containment. SQLite round-trip and late-response branch isolation tests also pass. No live model performance claim follows from these offline fixtures.

Validation of the preceding evidence-lifetime implementation on 2026-09-22: 82 focused tests and the full repository check passed. The offline regressions include a 76-request faux-provider conversation: initial excerpts appear in two projections, validation uses at most eight native operations, and a newly discovered file reaches a late request without exhausting the native budget. Additional regressions exercise native directory normalization, foreground grep locations, methods below thirty unrelated declarations, symbol ranking before truncation, overlapping read windows, and retirement after the offer limit. These fixtures do not measure live model quality or reasoning savings.

Validation of the preceding implementation on 2026-09-22: 75 focused tests and the full repository check passed. Offline regressions covered request-local cache reuse and invalidation, omission of cached skill evidence after external edits, native budget stops, mocked directory discovery, declaration/test selection from import-heavy symbol lists, and retirement after truncated foreground reads. A 13-request faux-provider conversation retained admitted evidence through its final boundary without exceeding the default native budget. That implementation did not expire repeatedly offered excerpts and its directory mock preserved a separator that native observations remove.
