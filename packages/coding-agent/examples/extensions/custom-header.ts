...
function getVoltMascot(theme: Theme): string[] {
	...
	const voltBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${voltBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${voltBlue(BLOCK.repeat(2))}    ${voltBlue(BLOCK.repeat(2))}`;
...
export default function (volt: ExtensionAPI) {
	...
	volt.registerCommand("builtin-header", {
