...
- **Evaluated:** pending, selected, abstained, unavailable, or cancelled, with elapsed evaluation time and fixed failure reasons. `abstained` means every validated choice was `none`, not that a read failed.
- **Selector input and choices:** the selector sees only new committed request text, not preceding conversation. The report shows offered skill/source counts and every validated answer (`skill=none`, `skill=skill-2 ("vitest")`, or `source-1=source-1`, for example). Skill names are bounded and JSON-escaped; source candidates use ordinals rather than paths. Failed evaluations have no valid choices. Decision application separately shows pending preparation, waiting for the initial projection, applying/applied, interrupted, or deterministic fallback.
- **Preparation operations:** existing managed skill/source reads and symbol queries, with pending/success/failure status, host reason codes, empty/truncated reads, and exact symbol-match counts. Publications show acceptance or the rejection reason; removals show abstention or changed skill selection. This separates a selected-but-unreadable skill, empty source, or rejected publication from Jev choosing `none`. These are production operations, not source-validation reads; no extra reads or inference run to produce the report.
...
- **Provider request observations:** the last eight conversational boundaries in the latest scope, their requested allowance, and a snapshot at the first `before_provider_request` observation for each boundary. Each snapshot records evaluation/preparation progress, choices and decision-application state at that observation, and existing host contribution states (`ready`, `admitted`, or `omitted`, with host reasons such as `source_unverified`). Earlier snapshots remain unchanged when a late selection refines a later request. A missing hook or failed diagnostic read is **unobserved**, not evidence of omission.
...
Only the latest request scope is retained, in memory. A new scope replaces it; a disabled request clears it. Reload, restart, session replacement, and tree navigation clear the report rather than reconstructing it from call history. Late results cannot populate a replacement report. Reports contain fixed contribution keys, candidate ordinals, bounded skill names, and diagnostic metadata, not request text, source paths/bodies, credentials, host resource IDs, or raw provider responses. Choices and operation details are runtime-only and are not added to persisted call history. `/jev status` remains the persisted branch-local **call history** view; those records alone cannot reconstruct preparation or admission.
...
- Export at most 8,192 request characters, 64 characters per skill name, and complete loaded skill descriptions, plus relative source paths/anchors. Descriptions are not truncated, so trailing applicability constraints remain available; skill bodies are not exported. Both serialized requests and response bodies are capped at 64 KiB. If complete metadata exceeds the request cap, skip evaluation before credential lookup or network access and retain deterministic fallback rather than truncate descriptions or drop candidates. Request-level truncation means context may be missing.
- Send one evaluation with at most three choice questions: one skill or `none`, and include/omit each offered source. Skill instructions require matching the requested task and current workflow phase, satisfying stated prerequisites rather than assuming them, and choosing `none` when no skill clearly applies. Shared terminology alone is insufficient: conducting a PR review is distinct from resolving an existing specific finding. These instructions guide selection, not guarantee its correctness. Validate every answer against its own finite option set and reject incomplete/extra/malformed answers atomically. Probabilities are ignored; they are not calibrated task-success confidence. Jev cannot invent a path, resource ID, or operation.
