 ...
 *
 * Usage: volt --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@earendil-works/volt-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/volt-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/volt-tui";
...
export default function (volt: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like volt-doom would need)
	volt.registerCommand("overlay-animation", {
	...
	volt.registerCommand("overlay-anchors", {
	...
	volt.registerCommand("overlay-margins", {
	...
	volt.registerCommand("overlay-stack", {
	...
	volt.registerCommand("overlay-overflow", {
	...
	volt.registerCommand("overlay-edge", {
	...
	volt.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	volt.registerCommand("overlay-maxheight", {
	...
	volt.registerCommand("overlay-sidepanel", {
	...
	volt.registerCommand("overlay-toggle", {
	...
	volt.registerCommand("overlay-passive", {
	...
	volt.registerCommand("overlay-focus", {
	...
	volt.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/volt/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/volt\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like volt-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (volt-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
