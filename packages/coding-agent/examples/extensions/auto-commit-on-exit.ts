...
export default function (volt: ExtensionAPI) {
	volt.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await volt.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[volt] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await volt.exec("git", ["add", "-A"]);
		const { code: commitCode } = await volt.exec("git", ["commit", "-m", commitMessage]);
