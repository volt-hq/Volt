 ...
 * - ~/.volt/agent/presets.json (global)
 * - <cwd>/.volt/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `volt --preset plan` - start with plan preset
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".volt", "presets.json");
...
export default function presetExtension(volt: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: volt.getThinkingLevel(),
				tools: volt.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			volt.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = volt.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				volt.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.volt/agent/presets.json or .volt/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await volt.setModel(originalState.model);
				}
				volt.setThinkingLevel(originalState.thinkingLevel);
				volt.setActiveTools(originalState.tools);
			} else {
				volt.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	volt.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = volt.getFlag("preset");
