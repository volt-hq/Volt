...
export default function planModeExtension(volt: ExtensionAPI): void {
	...
	function togglePlanMode(ctx: ExtensionContext): void {
		...
		if (planModeEnabled) {
			volt.setActiveTools(PLAN_MODE_TOOLS);
			ctx.ui.notify(`Plan mode enabled. Tools: ${PLAN_MODE_TOOLS.join(", ")}`);
		} else {
			volt.setActiveTools(NORMAL_MODE_TOOLS);
	...
	volt.registerShortcut(Key.ctrlAlt("p"), {
...
After completing a step, include a [DONE:n] tag in your response.`,
	...
	volt.on("agent_end", async (event, ctx) => {
		...
		if (choice?.startsWith("Execute")) {
			...
			if (refinement?.trim()) {
				volt.sendUserMessage(refinement.trim());
			}
		}
	});
