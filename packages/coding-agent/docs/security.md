...
- Pairing tickets are strict `volt+iroh://v2` payloads with a short-lived, one-time desktop pairing secret. Persisted daemon state stores its hash and non-secret metadata, not the raw secret. Legacy v1 or static-token ticket fields fail closed.
- Desktop-control authorization and relay-infrastructure enrollment are separate. The pairing secret, authenticated Iroh transport, host-observed client endpoint ID, and persisted RPC/tool grant authorize Volt. A relay pair grant only permits its exact host and phone endpoint IDs to register with the official relay fleet; it grants no workspace, RPC, or tool access.
- A managed QR includes a separate 10-minute enrollment claim ID and independent 256-bit claim secret, but no broker URL, Firebase credential, durable relay credential, or infrastructure bearer. The official app sends a limited-use App Check token only to its fixed broker and requires the approved relay origins to match the ticket exactly.
- Pair grants last 30 days and renew with seven days remaining. The pair-scoped renewal secret exists only in the app's Keychain-backed saved-host authority after the host handshake authenticates. Forget, abandoned adoption, supersession, and endpoint reset durably queue signed revocation before deleting local grant identity; startup and foreground activation retry it until acknowledged or expired.
...
- Desktop client revocation removes future Volt access from persisted state and asks a live host to close matching active streams, connections, and integrated runtimes when one is reachable. A revoked phone is blocked from every registered workspace in that state file. Pair-grant revocation is a separate broker operation and denies official relay registration for the affected host/phone pair.
...
Push notification delivery is mediated by the managed Volt relay by default. The mobile app registers its FCM token with the relay, then sends the desktop host only an opaque relay target id plus a target-scoped delivery credential. Volt host state stores that relay credential and optional token hash, but not the raw FCM registration token. Custom relays can be selected with `VOLT_PUSH_RELAY_URL`; if a custom relay uses shared bearer auth, pass it with `VOLT_PUSH_RELAY_AUTH_TOKEN`. Push registration keeps working after a TUI takes over a conversation because relayed `register_push_target` commands are forwarded to the daemon.

The daemon defaults to Iroh relay mode `production`, which emits a `volt-managed` descriptor for the fixed Volt-operated origins and uses endpoint enrollment rather than a client bearer. `VOLT_IROH_RELAY_MODE=development` emits the explicit `n0-public` descriptor for development and simulator testing; `disabled` is direct/LAN only. Setting `VOLT_IROH_RELAY_URLS` in production mode selects `custom-uncredentialed` owner-controlled origins instead of the managed fleet. Custom relays receive no App Check material.

Managed broker, App Check, signature, Firestore, origin-validation, or relay-callback failures deny new official relay registrations without disabling direct/LAN connectivity. Stock iroh-relay authorizes only when an endpoint registers, so grant expiry or revocation takes effect on the next registration (subject to the broker's short cache); it cannot interrupt an already-open registration or meter bytes per endpoint. Relay-wide concurrency/egress ceilings limit aggregate blast radius. See [Iroh relay enrollment design](https://github.com/volt-hq/Volt/blob/main/packages/coding-agent/docs/iroh-relay-enrollment-design.md) and [Self-hosted iroh relay](../../../docs/self-hosted-relay.md).
...
See [Using Volt](usage.md#remote-access-over-iroh-preview) for copy-pastable commands and [Iroh remote protocol v2](iroh-remote-protocol.md) for the external client contract.
