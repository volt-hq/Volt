...
- **`conversation_in_use` is retired.** Distinct paired devices now co-attach to one shared conversation runtime (or to a TUI-owned conversation over the daemon's byte relay). The old rejection implied a multi-user protection that never existed — all paired devices belong to the same human. Audit records (`~/.volt/agent/daemon/audit.jsonl`) cover pairing, lease transfers, and relay lifecycle so "what did the phone do while I was away" is reviewable after the fact.
