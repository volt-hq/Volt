...
- Same-client duplicates for one workspace/session are rejected with `duplicate_conversation_connection`; another client owning the same workspace/session is rejected with `conversation_in_use`. Different sessions in the same registered workspace may run concurrently.
...
Live Activity delivery is stream-bound. The app sends the host an activity ID, workspace name, session ID, platform, token environment, and a lowercase SHA-256 token hash that must match an existing delivery channel. The raw ActivityKit push token is not sent to the host. The host drops matching registrations on explicit unregister, selected-stream abort success, replacement, revocation, workspace unregister or authorization removal, and retained-runtime disposal.
