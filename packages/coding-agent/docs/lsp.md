...
- Other open files are eligible after a known-clean-to-failing transition, reported in a `Newly failing in other open files` section (capped at 5 files). Only files this session itself synced (or moved with an LSP edit) are eligible, so a server shared with subagents never attributes another session's open files to this session's edit. Each file carries its own freshness label; an unversioned dependent never inherits the edited file's `fresh` confidence. Current cross-file findings can still appear when the target file times out.
...
- One client runs per canonical `(server, server root)` pair. A failure in one nested root does not disable that server in another root.
- A session shares its language servers with its subagents, as long as they use the same project workspace and the same server settings (commands, file extensions, root markers, initialization options, server settings, `enabled`, `idleShutdownMs`, `traceFile`). Reporting settings such as `settleMs`, `severity`, `maxDiagnostics`, and `autoDiagnostics` stay per session and do not split servers. Each phone/daemon attach has its own set; unrelated sessions never share. Delivery history, failure messages, and install prompts stay per session, while server health, activity, and the failed-start breaker are shared.
- Servers shut down when the last session using them ends (including when `/new`, `/resume`, or `/fork` replaces it), and after `idleShutdownMs` without use (they respawn lazily on the next operation). `/reload` keeps healthy servers running when their server settings are unchanged, and clears failed-start state; changed server settings start a separate set.
- `/lsp` shows an on-demand health snapshot: configured unused/disabled servers and per-root starting, ready, degraded, failed, blocked, or idle records. Details include resolved executable, launch source, observed version, advertised capabilities, activity/latency counters, recent successes/failures, startup stderr, and request errors. `ready` means the transport initialized, not that build settings or indexing are verified. An alive process is not necessarily ready; an idle shutdown is not a failure. Unknown capabilities differ from an initialized server advertising none. There is no background status polling. `/lsp restart` stops the session's servers, including those shared with its subagents, and clears failed-start breakers so servers resolve and spawn fresh on next use.
