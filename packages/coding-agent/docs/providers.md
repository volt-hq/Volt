...
Volt supports subscription-based providers via OAuth and API key providers via environment variables or auth file. For each provider, volt knows all available models. The list is updated with every volt release.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.volt/agent/auth.json` and auto-refresh when expired.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](../../ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](../../ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.volt/agent/auth.json`:
...
volt --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal volt usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
volt --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Volt automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
