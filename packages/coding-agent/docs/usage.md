...
Each custom agent can select a separate model and thinking level with `model: provider/model-id` and `thinking: max` (or another supported level) in its definition. These settings apply to that child only; they do not change the parent's model, thinking level, or saved defaults. Thinking is clamped to the selected model's capabilities. Reload resources after editing definitions.
