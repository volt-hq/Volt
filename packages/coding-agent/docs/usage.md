...
In the TUI, the main background-job card shows the command or task, worker status, job duration, and the latest three non-empty output lines. Waiting calls identify the target job instead of showing a generic tool timer. Returned `jobs` inspections collapse to one line with the action, worker status, snapshot label, and a shortened job ID, without repeating the command or output. Ctrl+O expands the captured command, full job ID, and retained output. These post-hook snapshots remain fixed; launch cards, pending waits, and the `/jobs` inspector remain live. Running snapshots show their state "at capture" without an advancing timer. A completed inspection is not displayed as successful work when the worker is still running or has failed. Output previews are literal worker output, not inferred test progress. The original launch card also preserves the full submitted command or task.
