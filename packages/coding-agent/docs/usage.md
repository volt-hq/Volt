...
- The daemon defaults to Iroh relay mode `production`, using the Volt-operated relay fleet so saved-host reconnects survive restarts and network changes. The built-in Volt fleet requires a relay token provisioned by the fleet operator; set it with `VOLT_IROH_RELAY_AUTH_TOKEN` before first start, and the daemon persists it for later restarts. If no production token is available, set `VOLT_IROH_RELAY_MODE` to `disabled` for LAN-only connections or `development` for the public n0 development relays. Set `VOLT_IROH_RELAY_URLS` to use custom production relay origins, with `VOLT_IROH_RELAY_AUTH_TOKEN` when they require shared bearer authentication. Restart the daemon after changing relay environment variables.
