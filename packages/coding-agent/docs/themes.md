> volt can create themes. Ask it to build one for your setup.
...
Volt loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.volt/agent/themes/*.json`
- Project: `.volt/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `volt.themes` entries in `package.json`
...
On first run, volt detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.volt/agent/themes
vim ~/.volt/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, volt reloads it automatically for immediate visual feedback.
...
Volt uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, volt falls back to the nearest approximation.
