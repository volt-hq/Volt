...
```bash
npm install @earendil-works/volt-coding-agent
```
...
The `Agent` class (from `@earendil-works/volt-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@earendil-works/volt-coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@earendil-works/volt-coding-agent";
...
import { createAgentSession, defineTool } from "@earendil-works/volt-coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/volt-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/volt-coding-agent";
