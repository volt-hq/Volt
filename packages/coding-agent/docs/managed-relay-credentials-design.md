...
The response contains only the access JWT and access expiry. The refresh secret is unchanged. An inactive subscription returns `402 subscription_inactive` with a `Retry-After` paced by how long the grant's host has been without relay service, measured from its last JWT refresh (or creation, if it never refreshed): 15 seconds during the first day, 5 minutes during the first week, then one hour. A renewal notification restores the cached entitlement immediately, but only the suspended daemon can reconnect its host to the relay, and it learns of the renewal only by retrying; denied retries read cached state, so recently suspended hosts reconnect within seconds while Apple reconciliation keeps its separate one-hour cooldown. The daemon reports its next scheduled refresh in status, and a manual relay access check from `/remote` runs that same refresh early: it joins an in-flight refresh, skips the broker within 5 seconds of the previous attempt, follows the returned `Retry-After`, and cannot bypass the reconciliation cooldown or refresh rate limits. The denied request extends only the unrevoked endpoint/host inactivity deadline as a suspension heartbeat, mints no JWT, and lets the relay remove the expired JWT. Database acceptance is the linearization point: a concurrent transfer that commits later does not invalidate an access JWT already accepted for issuance.
