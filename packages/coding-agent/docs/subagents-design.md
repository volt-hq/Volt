...
Model-visible output is capped at 50 KB per task/step, while parallel results and registry list pages have a 100 KB aggregate cap; details payloads additionally bound task entries and retained output text so unbounded task counts stay under remote frame limits. Chain `{previous}` substitution uses that bounded output, XML-escapes it, and wraps it as untrusted prior data instead of forwarding raw child text. Delegation tool calls have no automatic deadline, token, or cost budget by default. The shared scope always accounts for aggregate consumption and applies the default structural depth/start/active-descendant safeguards. Independently, every child runtime warns at 80 turns and requires a tool-free report after turn 120. Finite host-provided `maxTotalTokens`, `maxTotalCostUsd`, or `maxDurationMs` values opt into whole-tree aborts; per-runtime turn thresholds can also be overridden. SDK callers may opt into a call-specific `runTimeoutMs`, and users or parents can cancel explicitly. Full child output remains in child session state when available; tool details include status, usage, truncation, error metadata, and a final snapshot of tree-wide accounting.
