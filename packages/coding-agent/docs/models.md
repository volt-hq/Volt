...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.volt/agent/models.json`.
...
Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so volt sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
For `models.json`, shell commands are resolved at request time. volt intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and volt cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps volt thinking levels to provider values and marks unsupported levels (see below) |
...
Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are volt thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`.
