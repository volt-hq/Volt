...
3. Starts an Iroh endpoint. The host defaults to `production` relay mode on the Volt-operated relay fleet so saved-host reconnects can survive host restarts; `VOLT_IROH_RELAY_MODE=disabled` is the explicit LAN-only opt-out.
...
```bash
volt remote host --workspace volt=. --allow-tools read,bash,edit,write,image_gen,web_search,web_fetch,grep,find,ls,inspect,lsp,subagent,subagent_registry,mcp
```

The default grant includes write, shell, image-generation, and active extension tools, and those unsafe tools require explicit host approval. The Codex-only `image_gen` tool can read and upload local reference images and write generated PNG files. TTY host and pair commands prompt before accepting `bash`, `edit`, `write`, or `image_gen`; noninteractive flows must pass `--yes`:
  ...
  "clients": [
    {
      ...
      "label": "Jordan iPhone",
      "allowedTools": "read,bash,edit,write,image_gen,web_search,web_fetch,grep,find,ls,inspect,lsp,subagent,subagent_registry,mcp",
...
- Pair a client and host across different networks using `production` relay mode.
- Send allowed remote RPC commands such as `get_state`, `get_transcript`, `list_sessions`, `prompt`, `abort`, `steer`, `follow_up`, `register_push_target`, and `extension_ui_response`.
...
| Native dependency increases install complexity | Keep `@number0/iroh` optional, keep native loading isolated from the main CLI, document native install troubleshooting, and reject standalone Node SEA remote host startup with actionable npm/source guidance |
...
- Remote clients use the default built-in tool grant (`read,bash,edit,write,image_gen,web_search,web_fetch,grep,find,ls,inspect,lsp,subagent,subagent_registry,mcp`) plus active extension tools unless configured otherwise. The `coding` and `full` RPC presets use this canonical default, including Codex-only `image_gen`, and clients keep their pair-time built-in tool grant on reconnect.
