...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".volt/skills")),
		source("user", join(homedir(), ".volt/agent/skills")),
		source("path", join(env.cwd, "../../../volt-skills")),
	],
	(skill, source) => ({ ...skill, source }),
);
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".volt/prompts")), source("user", join(homedir(), ".volt/agent/prompts"))],
