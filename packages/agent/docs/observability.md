...
# Volt Observability Design Notes

## Goal

Make `packages/ai` and `packages/agent`/harness observable without depending on OpenTelemetry, Sentry, or any APM vendor.

Volt should emit stable, structured lifecycle events. External listeners can convert those events into OTel spans, Sentry spans, logs, metrics, or custom telemetry.
...
```text
traceId=t1 spanId=s1 parent=-  name=volt.agent.prompt
traceId=t1 spanId=s2 parent=s1 name=volt.agent.turn
traceId=t1 spanId=s3 parent=s2 name=volt.ai.provider.request
traceId=t1 spanId=s4 parent=s2 name=volt.agent.tool_call
traceId=t1 spanId=s5 parent=s4 name=volt.session.append_entry
```
...
```ts
await Promise.all([
  runWithVoltContext({ userId: "alice" }, () => harness.prompt("A")),
  runWithVoltContext({ userId: "bob" }, () => harness.prompt("B")),
]);
```

Deep code can then read the correct current context for the active async chain.

Volt must run in Node, Bun, browser, workers, and other JS runtimes, so ALS cannot be the core abstraction. It should be a runtime adapter.

## Core design

Volt owns a small runtime-agnostic observability abstraction:

```ts
export interface VoltObservabilityContext {
  traceId?: string;
  currentSpanId?: string;
  userContext?: Record<string, unknown>;
}

export interface VoltObservabilityEvent {
...
export interface VoltObservability {
  getContext(): VoltObservabilityContext | undefined;
  runWithContext<T>(context: VoltObservabilityContext, fn: () => T): T;
  emit(event: VoltObservabilityEvent): void;
...
```ts
export function configureVoltObservability(observability: VoltObservability): void;
export function subscribeVoltObservability(listener: (event: VoltObservabilityEvent) => void): () => void;
export function runWithVoltContext<T>(userContext: Record<string, unknown>, fn: () => T): T;
...
channel("volt.observability").publish(event);
```

Subscribers can create OTel/Sentry spans without monkey-patching volt.

## What volt emits

Volt emits what happened. It does not create OTel/Sentry spans directly.
...
volt.agent.prompt
volt.agent.skill
volt.agent.prompt_template
volt.agent.compaction
volt.agent.branch_navigation
volt.agent.session.append_entry
volt.ai.provider.request
```
...
```text
volt.agent.turn
volt.agent.tool_call
volt.agent.queue_update
volt.ai.provider.retry
volt.ai.provider.first_token
volt.ai.provider.usage
volt.session.read
volt.session.write
```
...
Observability must never affect volt execution.
...
```text
packages/ai
  emits volt.ai.* events

packages/agent
  emits volt.agent.* / volt.session.* events
```
...
packages/otel
  subscribes to volt events and creates OpenTelemetry spans
```

## Thesis

Volt defines a stable, safe event contract. Adapters define where events go.
