...
describe("TUI.queryTerminalBackgroundColor", () => {
	...
	it("does not let a timed-out OSC 11 query consume the next query reply", async () => {
		...
		try {
			...
			assert.strictEqual(await timedOutQuery, undefined);

			const nextQuery = tui.queryTerminalBackgroundColor({ timeoutMs: 1000 });
			terminal.sendInput("\x1b]11;#000000\x07");

			assert.deepStrictEqual(await nextQuery, { r: 0, g: 0, b: 0 });
		} finally {
			tui.stop();
		}
	});
