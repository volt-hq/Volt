...
import {
	getCloudflareAiGatewayWorkersModel,
		...
		for (const block of response.content) {
			if (block.type === "text") {
				...
				if (typeof a !== "number" || typeof b !== "number" || typeof operation !== "string") {
					throw new Error("math_operation returned invalid arguments");
				}
				let result: number;
				switch (operation) {
					case "add":
...
describe("Generate E2E Tests", () => {
	...
	describe.skipIf(!hasCloudflareAiGatewayCredentials())(
		...
		() => {
			const llm = getCloudflareAiGatewayWorkersModel();
	...
	describe.skipIf(!process.env.ZAI_API_KEY)("zAI Provider (glm-5.3 via OpenAI Completions)", () => {
	...
	// =========================================================================
	// OAuth-based providers (credentials from ~/.volt/agent/oauth.json)
	...
	describe.skipIf(!hasBedrockCredentials())("Amazon Bedrock Provider (claude-opus-4-6 interleaved thinking)", () => {
		...
		it("should pass requestMetadata to the SDK payload", { retry: 3 }, async () => {
			...
			let capturedPayload: unknown;
			const metadata = { app: "volt-test", env: "ci" };
			const response = await complete(
				llmSonnet,
				{
					messages: [
	...
	if (!process.env.VOLT_NO_LOCAL_LLM) {
		try {
