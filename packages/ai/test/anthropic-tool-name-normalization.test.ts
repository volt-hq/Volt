...
describe.skipIf(!oauthToken)("Anthropic OAuth tool name normalization", () => {
	...
	it("should handle volt's built-in tools (read, write, edit, bash)", async () => {
		// Volt's tools use lowercase names, CC uses PascalCase
		const readTool: Tool = {
			name: "read",
	...
	it("should NOT map find to Glob - find is not a CC tool name", async () => {
		// Volt has a "find" tool, CC has "Glob" - these are DIFFERENT tools
