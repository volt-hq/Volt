...
- Exception for fresh worktrees: when workspace packages have no `dist/` (so `npm run check` or the pre-commit hook fails on unresolved `@hansjm10/*` imports), you may build only the missing packages from their package roots, in dependency order (e.g. `cd packages/ai && npm run build`, then `packages/agent`). `dist/` is git-ignored; never commit build output.
- Local validation requires only tests affected by the change, including any created or modified tests. Do not run the full test suite unless the user explicitly requests it; a full-suite run is not required before opening a PR. For docs-only changes, validate relevant links, examples, and metadata instead of running unrelated runtime tests.
- Never run the full Vitest suite directly: it includes e2e tests that activate when endpoint/auth env vars are present. Run affected tests from the package root using an installed Vitest CLI, or use `./test.sh` from the repo root with workspace and test-file filters. If the user requests the full non-e2e suite, use `./test.sh` without filters.
