...
# Verified from the signed TestFlight build 4 application-identifier.
APP_ATTEST_APP_ID="${VOLT_APP_ATTEST_CANARY_APP_ID:-FLCDL5CJU2.com.hansjm10.volt}"
...
Environment overrides:
  ...
  VOLT_APP_ATTEST_CANARY_APP_ID     exact signed App ID (default: FLCDL5CJU2.com.hansjm10.volt)
...
require_app_store_authority() {
	if [[ ! "$APP_ATTEST_APP_ID" =~ ^[A-Z0-9]{10}\.com\.hansjm10\.volt$ ]]; then
		echo "App Attest canary App ID is invalid" >&2
		exit 1
	fi
